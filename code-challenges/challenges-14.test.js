...
const screenForNames = (arr) => {
  ...
  return arr.filter(item => /^((Mr||Mrs||Ms||Dr).\s)[A-Z]/.test(item));
...
const toTitleCase = (arr) => {
  ...
  let upper=arr.map(item=>{
    return item.replace(regex,item[0].toUpperCase())
  })
  return upper;
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  const heavierArr = arr.reduce((name, mass) => {
    if (parseInt(mass.mass) > parseInt(arr[0].mass)) {
      if (name) {
        name += ' - ';
      }
      name += mass.name;
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i = 0; i < board.length; i++) {
    if (x[0] == y[0] == z[0] || x[0] == y[1] == z[2] || x[2] == y[1] == z[0] || x[0] == x[1] == x[2] || y[0] == y[1] == y[2] || z[0] == z[1] == z[2] || x[1] == y[1] == z[1] || x[2] == y[2] == z[2]) {
      return true
    } else {
      return false
    }
