...
const charCode = (arr) => {
  // Solution code here...
  let val;
  arr.map(item=>{ val= item.charCodeAt(0);
    return val;});
};
...
const extractAbilities = (arr) => {
  // Solution code here...
  snorlaxAbilities.abilities.map((item)=>{
    let val=item.snorlaxAbilities.abilities.name;
...
const extractStats = (arr) => {
  ...
  snorlaxStats.map((item)=>{
    let dat=[];
    dat.name=item.snorlaxAbilities.abilities.name;
    dat.total=item.snorlaxAbilities.abilities.effort+item.snorlaxAbilities.abilities.weight;
return dat;
  })
};
