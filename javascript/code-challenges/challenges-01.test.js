...
const speaker = (words, callback) => {
  // Solution code here...
  let newArr=[];
  words.forEach(value =>{let newValue=callback(value);
  newArr.push(newValue);
  })
  return newArr;
};
...
const createList = (availableItems) => {
  ...
  availableItems.forEach(value=>{
    if(value.available==true){newArr.push(value.name)}
  })
  return newArr;
};
...
const fizzbuzz = (arr) => {
  ...
  const fizzbuzz = (arr) => {
    ...
    let newArray = [];
    arr.forEach(value =>{
      if (value % 5 ==0 && value % 3 ==0){newArray.push("Fizz Buzz")}
      else if (value % 5 ==0){newArray.push("Buzz")}
      else if (value % 3 ==0){newArray.push("Fizz")}
